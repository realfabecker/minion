...
download_file=minion-0.0.1-linux-amd64.tar.gz
repo_url=https://api.github.com/repos/realfabecker/minion/releases/latest
...
check_profile_config() {
  if [[ $(command -v minion && echo "ok" || echo "no") == "no" ]]; then
    p="$PATH"
    cat <<EOF
...
install_latest_release() {
  ...
  tar -C "${install_dir}" -xf "${download_dir}/${download_file}" minion
}

download_latest_release
install_latest_release
info "Completed" "minion installation at $install_dir/minion"
check_profile_config